...
namespace Minimis.Items.Weapons
{
	public class WoodenBaton : ModItem
	{
		public override void SetStaticDefaults() {
			Tooltip.SetDefault("Pow! Bam!");  //The (English) text shown below your weapon's name
		}

		public override void SetDefaults() {
			item.damage = 5; // The damage your item deals
			...
			item.value = Item.buyPrice(copper: 12); // The value of the weapon in copper coins
			item.rare = ItemRarityID.White; // The rarity of the weapon, from -1 to 13. You can also use ItemRarityID.TheColorRarity
			item.UseSound = SoundID.Item1; // The sound when the weapon is being used
			item.autoReuse = false; // Whether the weapon can be used more than once automatically by holding the use button
			item.crit = 5; // The critical strike chance the weapon has. The player, by default, has 4 critical strike chance
		...
		public override void AddRecipes() {
			...
			recipe.AddTile(TileID.WorkBenches); // Set the crafting tile to ExampleWorkbench
